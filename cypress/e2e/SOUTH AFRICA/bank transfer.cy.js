describe('Payment Validation SA', () => {
  ...
  cy.wait(20000)
  cy.get(':nth-child(3) > .relative.flex-col > .card', { timeout: 50000 })
  .should('exist')
  .should('be.visible')
  .scrollIntoView()
  .click({ force: true });
  ...
  .type('0615330684', { force: true });
                    // Type the phone number

  cy.get('#email').type('eniolaomoyanola@gmail.com')
  cy.get('.pt-5 > .w-full.justify-between > :nth-child(2) > .flex', { timeout: 50000 })
  .should('exist')
  .should('be.visible')
  .scrollIntoView()
  .click({ force: true });
...
//asserting user can proceed to pay with capitec pay
  ...
  if ($body.find('#radix-\\:rd\\: > div > button').length) {
    cy.get('#radix-\\:rd\\: > div > button').click({ force: true });
  } else {
