describe('Payment Validation SA', () => {
    ...
    it('validates payment flow in SA', () => {
      ...
      cy.contains('Morocco')
        ...
        //local card
        cy.contains('.card-content', 'Credit / Debit Card')
  .should('be.visible')
  .click();
  ...
  cy.get('#cardHolderName', { timeout: 50000 }).scrollIntoView().should('be.visible').type('Eniola',{ force: true })
  cy.get('#cardNumber').type('5438898014560229')
  cy.get('#expiryDate')
  .scrollIntoView()
  .type('3110', { force: true })
  cy.get('#cvv')
  .scrollIntoView()
  .type('564', {force: true })
...
//asserting user can proceed to pay with local card
